from transfer_mod import manual_updates, mod_time_display
from tkinter import *
...
check = Button(root, text = "back up", command = manual_updates)
check.grid(row = 1, column = 0, rowspan = 2, columnspan = 2)
check_all = Button(root, text = "back up all")
check.grid(row = 2, column = 0, rowspan = 2, columnspan = 2)
...
for x in doc_nameB:
    #time = datetime.fromtimestamp(os.stat(fileB+"/"+x).st_mtime)
